...
def coding_prediction(rdata,idata,outfile):
	...
	print >>RCMD, 'test <- read.table(file=\"%s\",sep="\\t",col.names=c("ID","mRNA","ORF_start","ORF","ORF_seq","Fickett","Hexamer"), stringsAsFactors=F)' % (idata)
	print >>RCMD, 'test$prob <- predict(mylogit,newdata=test,type="response")'
	print >>RCMD, 'attach(test)'
	print >>RCMD, 'output <- cbind("mRNA_size"=mRNA,"ORF_starting_position"=ORF_start,"ORF_size"=ORF,"ORF_sequence"=ORF_seq,"Fickett_score"=Fickett,"Hexamer_score"=Hexamer,"coding_prob"=test$prob)'
...
def extract_feature_from_bed(inbed,refgenome,stt,stp,c_tab,g_tab,min_length,kind):
	...
	if inbed.strip():
		...
		for i in tmp.all_orfs(direction="+",start_coden=stt_coden, stop_coden=stp_coden, min_length=min_length, kind=kind):
			(CDS_size, CDS_frame, CDS_seq, CDS_start) = i
			fickett_score = fickett.fickett_value(CDS_seq)		
			hexamer = FrameKmer.kmer_ratio(CDS_seq,6,3,c_tab,g_tab)
			#print CDS_seq
			features.append( (geneName, mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer) )
		return features

def extract_feature_from_seq(seq,stt,stp,c_tab,g_tab,min_length,kind):
	...
	for i in tmp.all_orfs(direction="+",start_coden=stt_coden, stop_coden=stp_coden, min_length=min_length, kind=kind):
		...
		hexamer = FrameKmer.kmer_ratio(CDS_seq,6,3,c_tab,g_tab)
		#print CDS_seq
		features.append( (mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer) )
	return features
...
def main():
	...
	parser.add_option("-o","--outfile",action="store",dest="out_file",help="output file. Tab separated text file: geneID <tab> mRNA size <tab> ORF sequence <tab> ORF position <tab> ORF size <tab> Fickett Score <tab> Hexamer Score<tab>Coding Probability.")
	...
	if file_format == 'UNKNOWN':
		...
		for line in ireader.reader(options.gene_file):
			...
			for i in extract_feature_from_bed(line, options.ref_genome, options.start_codons, options.stop_codons,coding,noncoding, int(options.min_length), str(options.kind)):
				(gene_id, mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer) = i
				print >>TMP, '\t'.join([str(i) for i in [gene_id, mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer]])
		...
		for sname,seq in FrameKmer.seq_generator(options.gene_file):
			...
			for i in extract_feature_from_seq(seq = seq, stt = options.start_codons,stp = options.stop_codons,c_tab=coding,g_tab=noncoding, min_length=int(options.min_length), kind=str(options.kind)):
				(mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer) = i
				print >>TMP, '\t'.join(str(i) for i in (sname, mRNA_size, CDS_start, CDS_size, CDS_seq, fickett_score, hexamer))
			print >>sys.stderr, "%d genes finished\r" % count,
	TMP.close()
