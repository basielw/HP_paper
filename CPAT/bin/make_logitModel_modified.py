...
def make_logit(infile,rscript, outfile,head):
	'''make logit model'''
	RCMD = open(rscript,'w')
	print >>RCMD, 'data <- read.table(file=\"%s\",sep="\\t",header=T)' % (infile)
	print >>RCMD, 'attach(data)'
	print >>RCMD, 'mylogit <- glm(%s ~ %s + %s + %s + %s, family=binomial(link=\"logit\"), na.action=na.pass)' % ("Label","mRNA","ORF","Fickett","Hexamer")
	print >>RCMD, 'save.image(\"%s\")' % (outfile)
	RCMD.close()
	try:
...
def main():
	...
	######################################################################################
	count=0		
	print >>sys.stderr, "Process protein coding transcripts: " + options.coding_file
	...
	######################################################################################
	count=0
	print >>sys.stderr, "Process non coding transcripts: " + options.noncoding_file
	...
	# writing data
	######################################################################################	
	print >>TMP, '\t'.join(header)
	for i in train_data:
		print >>TMP, '\t'.join([str(j) for j in i])	
	TMP.close()
	
	print >>sys.stderr, "build logi model ..."
